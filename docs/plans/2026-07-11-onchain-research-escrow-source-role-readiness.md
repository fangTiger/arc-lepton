...
本文档最初是一份本地 readiness 清单；当前已追加真实执行后的完成口径。`configure_sources_and_roles` 已在 chainId 5042002 获授权后广播并验证：一次性 bindFactory、五个 source 登记、角色 grant/revoke、deployer 撤权与 finalized block readback 已有公开证据。三个核心合约 Explorer exact-match source/ABI 也已完成，并记录到 `deployments/5042002.json.sourceVerification`；不得把后续 rollout/E2E/rollback 证据与 13.4 混为一谈。
...
authorization package、handoff、briefing、requestDigest 不是授权记录。它们只能帮助整理公开确认材料，不能替代用户对 `configure_sources_and_roles` 或 Explorer exact-match source/ABI 发布的明确回复。现有配置/角色执行证据覆盖 bindFactory、Registry/Factory 双向 wiring、五个 source 登记、grant/revoke、deployer 撤权与 finalized block 读回；source verification 证据覆盖 Arcscan exact-match source/ABI，授权 requestDigest 为 `sha256:f575600e834861c57a53d0c3394ed741732e0e4b43575904a3848c17ad68d39f`。任何新的外部写入仍需独立明确授权，用户未回应或模糊同意必须停止，request/commit/address/source/role/gas 变化必须重新授权。
...
候选 manifest 或本地 readiness 不得替代 13.4 真实执行证据。13.4 现已具备以下公开复核证据：
...
6. 上述全部证据进入最终 `deployments/5042002.json` manifest，并由独立公开 RPC verifier 复核。
...
- `deployments/5042002.json.sourceVerification`：Arcscan Explorer exact-match source/ABI 证据，覆盖 Registry、implementation、Factory 的 address/name/filePath/compiler/optimizer/runs/evm/constructorArgs/sourceCodePresent/abiPresent/isFullyVerified/url。
...
因此 `openspec/changes/onchain-research-escrow/tasks.md` 的 13.4 已勾选完成；剩余外部/live 工作转入 14.2–14.4、14.7 和 14.9。
