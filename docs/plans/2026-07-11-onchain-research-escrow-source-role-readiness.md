...
本文档只是一份本地 readiness 清单，用于把 `configure_sources_and_roles` 阶段的授权范围、执行顺序和证据边界整理清楚。当前没有执行任何外部写入：不得 source verify、不得 source 登记、不得 grant/revoke、不得角色移交，也不得把任何候选 manifest 或本地 readiness 当作已完成证据。
...
authorization package、handoff、briefing、requestDigest 不是授权记录。它们只能帮助整理公开确认材料，不能替代用户对 `configure_sources_and_roles` 的明确回复，不能替代 Explorer exact-match source/ABI，不能替代一次性 bindFactory、Registry/Factory 双向 wiring、五个 source 登记、grant/revoke、deployer 撤权或 finalized block 读回。用户未回应或模糊同意必须停止；request/commit/address/source/role/gas 变化必须重新授权。
...
候选 manifest 或本地 readiness 不得替代 13.4 真实执行证据。13.4 只有在授权后真实完成以下证据并可公开复核时才能勾选：
...
- 用户未对 `configure_sources_and_roles` 给出当次明确授权。
- 没有真实 source verification、source registration、role grant/revoke、role transfer 或 deployer revoke 交易。
- 没有 finalized block 上的 Registry/Factory wiring、source 配置、role graph 或 Explorer exact-match 公开证据。
- 没有最终 `deployments/5042002.json` manifest 或 13.6 final public verifier 成功结果。

因此本文件不改变 `openspec/changes/onchain-research-escrow/tasks.md`；13.4 保持未完成。
