...
2. ARC receipt 配置
   ...
   - `ARC_RECEIPT_MODE=arc`：直接 `/api/data/*` 付费 API 会发送一笔 0-value、带 JSON calldata 的 ARC receipt 交易，并等待 receipt。
   - `ARC_RECEIPT_MODE=arc` 下直接付费 API 每次请求都必须显式提供幂等 key：优先 `Idempotency-Key`，其次 `X-Idempotency-Key`，也支持 `requestId` query 参数；幂等 key 必须为 1-128 个字符，且仅允许 `[A-Za-z0-9._:-]`；缺失或非法时请求会直接失败，不会广播。
   ...
   - Research agent 每次 research 默认最多执行 3 个付费数据源调用；工具调用会先写入本地 `pending` payment intent，不在回答主路径等待 ARC RPC、交易广播或出块确认。
   - 同一次 research 的多个 pending intents 会在报告完成后异步聚合成最多一笔 `arc-lepton.research-settlement` ARC transaction；确认后这些逻辑 tx_log 共享同一个 `txHash`、`chainId` 和 `blockNumber`。
   - Settlement 失败不会撤回已完成报告；TX feed、详情页和钱包 API 会按真实状态显示 `pending settlement`、`confirmed`、`failed` 或 `mock receipt`。TX feed 会低频同步 `tx_log` 直到 payment 状态进入终态。失败状态保留错误原因，后续补偿任务可基于 `payment_settlement`/pending 状态重试。
...
3. 安装依赖 + 推 schema 到 DB
   ...
   - 如果拉取了 research 批量异步 settlement 功能，也需要执行 `pnpm db:push`：它会给 `tx_log` 增加可为空的 `settlement_id`，并创建 `payment_settlement` 表用于 claim、确认、失败记录和补偿重试扫描。
