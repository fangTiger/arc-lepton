...
test("rollback drill records the current OpenSpec progress", async () => {
  ...
  assert.ok(drill.includes("102/107 tasks"), "rollback drill must mention 102/107 tasks");
  assert.doesNotMatch(drill, /(?:9[3-7]|100)\/107 tasks/, "rollback drill must not mention stale 93-100/107 progress");
});
